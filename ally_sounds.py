...
class Mob(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pygame.transform.scale(taco_img, (75,75))
        self.image.set_colorkey(BLACK)
        self.rect = self.image.get_rect()
...
# ally added for a start screen
def show_go_screen():
    screen.blit(background, background_rect)
    draw_text(screen, "Llama's love TACOS!", 64, WIDTH / 2, HEIGHT / 4)
...
#load game sounds
eat_game = pygame.mixer.Sound(path.join(snd_dir, 'apple_bite.wav'))
...
while running:
    ...
    # check to see if a taco hit the player /Changed "false to true"
